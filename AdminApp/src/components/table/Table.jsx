...
const List = ({ type }) => {
  ...
  const transferIdRoomToRoomNumber = (roomsId, dataRooms) => {
    let roomNumbers = [];
    for (let i = 0; i < roomsId.length; i++) {
      const number = dataRooms.reduce((result, d) => {
        if (d.roomNumbers.map((r) => r._id).includes(roomsId[i])) {
          const roomNumber = d.roomNumbers.find(
            (roomNumber) => roomNumber._id === roomsId[i]
          );
        ...
        <TableBody>
          {dataTransactions?.map((transaction) => (
            <TableRow key={transaction._id}>
              ...
              <TableCell className={classes.tableCell}>
                {transferIdRoomToRoomNumber(transaction.room, rooms).join(", ")}
              </TableCell>
              <TableCell className={classes.tableCell}>
